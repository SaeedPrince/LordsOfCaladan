...
#include "BastudioStrategyCameraComponent.h"


// Sets default values for this component's properties
UBastudioStrategyCameraComponent::UBastudioStrategyCameraComponent()
{
...
void UBastudioStrategyCameraComponent::BeginPlay()
{
	Super::BeginPlay();
...
// Called every frame
void UBastudioStrategyCameraComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
void UBastudioStrategyCameraComponent::Activate(bool bReset)
{
...
// Functions
void UBastudioStrategyCameraComponent::SetBoundingBoxReference(class ABeastudioBoundingBox* GivenBoundingBox)
{
...
void UBastudioStrategyCameraComponent::_InitialValues()
{
	PrimaryComponentTick.bCanEverTick = true;
	bAutoActivate = false;
...
void UBastudioStrategyCameraComponent::_ConstructionScript()
{
...
void UBastudioStrategyCameraComponent::_SetTargetZ()
{
...
void UBastudioStrategyCameraComponent::_SetAdjustedMovementSpeed()
{
	AdjustedMovementSpeed = (TargetDistance - MinDistance) / (MaxDistance - MinDistance) * MovementSpeedRange + MovementSpeed;
}

void UBastudioStrategyCameraComponent::MoveCamera(float X, float Y, bool SetRotation, float Rotation, bool SetDistance, float Distance, bool Teleport)
{
...
bool UBastudioStrategyCameraComponent::IsCameraMoving()
{
	return IsMoving;
}

void UBastudioStrategyCameraComponent::SetInputEnabled(bool InputEnabled)
{
	IsInputEnabled = InputEnabled;
}

void UBastudioStrategyCameraComponent::_AddRotation(float Rotation)
{
	if (Rotation != 0.0f)
	{
...
void UBastudioStrategyCameraComponent::_SetTargetXY(FVector DeltaLocation)
{
...
void UBastudioStrategyCameraComponent::_AddMovementX(float X)
{
	if (X != 0.0f)
	{
...
void UBastudioStrategyCameraComponent::_AddMovementY(float Y)
{
	if (Y != 0.0f)
	{
...
void UBastudioStrategyCameraComponent::_UpdateMovement()
{
...
void UBastudioStrategyCameraComponent::_UpdateRotation()
{
...
void UBastudioStrategyCameraComponent::_UpdateZoom()
{
...
void UBastudioStrategyCameraComponent::_ProcessMouseWindowBorderScroll()
{
...
void UBastudioStrategyCameraComponent::CameraMoveForward(float Value)
{
	if (IsInputEnabled)
	{
		_AddMovementX(Value);
...
void UBastudioStrategyCameraComponent::CameraMoveRight(float Value)
{
	if (IsInputEnabled)
	{
		_AddMovementY(Value);
...
void UBastudioStrategyCameraComponent::CameraRotate(float Value)
{
	if (IsInputEnabled)
	{
		_AddRotation(Value);
...
void UBastudioStrategyCameraComponent::CameraZoomIn()
{
	if (IsInputEnabled)
	{
...
void UBastudioStrategyCameraComponent::CameraZoomOut()
{
	if (IsInputEnabled)
	{
		TargetDistance = FMath::Clamp(TargetDistance + ZoomSpeed, MinDistance, MaxDistance);
